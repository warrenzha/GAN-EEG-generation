...
parser.add_argument("--lr1", type=float, default=0.00005, help="adam: learning rate generator")
parser.add_argument("--lr2", type=float, default=0.0001, help="adam: learning rate discriminator")
...
# def weights_initConvTrans1d(m):
#     torch.nn.init.uniform(m.biasout, a=0, b=1)
#     print(m.biasout)
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        # self.biasout = nn.Parameter(torch.tensor([0], dtype=torch.float32, device='cuda', requires_grad=True))
        self.nz = opt.nz
        self.layer1 = nn.Sequential(
            nn.Linear(self.nz, 400),
            nn.PReLU()
        )
        self.layer2 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=16, out_channels=16, kernel_size=8, stride=2),
            nn.PReLU()
        )
        ...
        self.layer4 = nn.Sequential(
            nn.ConvTranspose1d(in_channels=16, out_channels=8, kernel_size=6, stride=2)
            # nn.PReLU()
        )

    def forward(self, z):
        out = self.layer1(z)
        out = out.view(out.size(0), 16, 25)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv1d(in_channels=8, out_channels=16, kernel_size=6, stride=2),
            nn.PReLU()
        )
        self.layer2 = nn.Sequential(
            nn.Conv1d(in_channels=16, out_channels=16, kernel_size=8, stride=2),
            nn.PReLU(),
            nn.MaxPool1d(2)
        )
        # self.layer3 = nn.Sequential(
...
# torch.nn.init.uniform(generator.biasout, a=0, b=1)
# print(generator.biasout)

discriminator.to(device)
generator.to(device)

# aaa = torch.rand(1, 200)
#
# writer = SummaryWriter(comment='test_your_comment')
# writer.add_graph(generator, aaa)


summary(generator, (1, 100))
summary(discriminator, (8, 240))
